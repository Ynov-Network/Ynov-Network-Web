...
const AppHeader = () => {
  ...
  return (
    <header className="bg-white/80 backdrop-blur-xl border-b border-gray-200/50 px-6 py-4 sticky top-0 z-20 shadow-sm">
      <div className="flex items-center justify-between w-full">
        {/* Enhanced Search Bar - now takes full left space */}
        <div className="flex-1 max-w-2xl">
          <div className="relative group">
            <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5 group-hover:text-ynov-primary transition-colors duration-300" />
            <Input
              placeholder="Search for friends, posts, topics..."
              className="pl-12 h-12 bg-gray-50/80 border-0 focus:bg-white transition-all duration-300 w-full rounded-xl shadow-sm hover:shadow-md focus:shadow-lg"
            />
        ...
        <div className="flex items-center space-x-4 ml-8">
          ...
          <Popover>
            ...
            <PopoverContent className="w-96 p-0 shadow-xl border-0 rounded-xl" align="end">
              <div className="p-4 border-b bg-gradient-to-r from-ynov-primary to-ynov-secondary">
              ...
              <div className="max-h-96 overflow-auto">
                {notifications.map((notification) => (
                  <div key={notification.id} className={`p-4 border-b hover:bg-gray-50 transition-colors duration-200 ${!notification.read ? 'bg-blue-50/50' : ''}`}>
                    <div className="flex items-center space-x-3">
                      <div className="relative">
                        ...
                        {notification.count && notification.count > 1 && (
                          <Badge className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-ynov-primary text-white text-xs p-0 flex items-center justify-center">
                            +{notification.count}
                          </Badge>
                        )}
                      </div>
              ...
              <div className="p-4 border-t bg-gray-50">
                <Link to="/notifications">
                  <Button variant="ghost" className="w-full text-center hover:bg-white transition-colors">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Avatar className="h-9 w-9 hover:ring-2 hover:ring-ynov-primary/20 transition-all duration-200 cursor-pointer">
            ...
            <PopoverContent className="w-64 p-0 shadow-xl border-0 rounded-xl" align="end">
              ...
              <div className="py-2">
                <Link to={`/profile/${user?.username}`}>
                  <Button variant="ghost" className="w-full justify-start px-4 hover:bg-gray-50 transition-colors">
                ...
                <Button
                  variant="ghost"
                  className="w-full justify-start px-4 hover:bg-gray-50 transition-colors"
